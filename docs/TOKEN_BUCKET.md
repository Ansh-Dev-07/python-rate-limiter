...
Current Algorithm Version: **v0.7.0**
...
Each user maintains an independent token bucket state.
This naturally supports multi-user rate limiting and allows the project to use either in-memory or Redis-backed storage.
...
The `TokenBucket` algorithm itself does not require a specific storage mechanism.
In the project's in-memory implementation, bucket state is stored in a Python `TokenBucket` object.
The Redis backend stores equivalent bucket state externally using Redis and performs the refill and request decision inside a Lua script.
The storage mechanism can therefore change without changing the fundamental Token Bucket algorithm.

---

## Single-Process and Shared-State Scope

The in-memory implementation provides thread-safe rate limiting within a single Python process.
Separate application instances using the in-memory backend maintain separate rate-limit state.
The Redis backend stores bucket state externally and performs rate-limit processing atomically in Redis, providing a foundation for sharing rate-limit state across multiple application instances.
However, the project has not yet demonstrated a complete distributed deployment.

---

## Time Dependency

The in-memory `TokenBucket` implementation uses Python's `time.time()` to calculate elapsed time.
If the system clock changes unexpectedly, elapsed-time calculations may be affected.
The Redis backend uses Redis `TIME` for Redis-backed bucket calculations instead of relying on the application server's local clock.
A future improvement to the in-memory implementation could use a monotonic clock such as `time.monotonic()` for elapsed-time measurement, since monotonic clocks are not affected by system clock adjustments.

---

## Future Improvements

The current implementation intentionally focuses on the Token Bucket algorithm while allowing its surrounding infrastructure to evolve independently.
...
In the project's multi-user implementation, one bucket state is maintained for each user by the configured backend.
For `InMemoryBackend`, this means one `TokenBucket` object per user.
Therefore, the in-memory implementation's overall memory usage is:
...
Instead, each request updates only the bucket state belonging to the requesting user.
For the in-memory implementation, only the corresponding `TokenBucket` is processed.
For the Redis implementation, only the corresponding user's Redis bucket state is processed by the Lua script.
This keeps the rate-limiting computation associated with an individual request independent of the total number of users or previous requests.
The exact end-to-end performance characteristics depend on the configured backend and, for Redis, also include communication with the Redis server.

---

## Performance Characteristics

For the project's in-memory implementation, the multi-user bucket lookup and creation operations use Python dictionary operations with average constant-time complexity.

The following complexity characteristics describe the project's in-memory implementation:
...
With FastAPI providing an HTTP integration in v0.6.0 and the backend abstraction with Redis support introduced in v0.7.0, the surrounding architecture can continue evolving with Docker, CI/CD, and future infrastructure improvements while the underlying Token Bucket algorithm remains unchanged.
...
The Token Bucket algorithm therefore serves not only as the foundation of this project, but also as an excellent example of how a simple algorithm can evolve into a production-focused backend component through thoughtful engineering and incremental design.